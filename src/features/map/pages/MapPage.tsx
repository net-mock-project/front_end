...
export default function MapPage() {
	...
	useEffect(() => {
		if (!location || !map || hasCentered.current) return;
		...
		<section className="map-stage">
			<div className="map-toolbar">
				<div className="map-toolbar-actions">
					<Button className={showVolunteers ? "is-selected" : ""} icon={<UserOutlined />} onClick={() => setShowVolunteers(!showVolunteers)}>Volunteer ({volunteers.length})</Button>
					<Button className={showRequests ? "is-selected" : ""} icon={<ThunderboltFilled />} onClick={() => setShowRequests(!showRequests)}>Yêu cầu ({activeRequests.length})</Button>
					<Button icon={<AimOutlined />} onClick={() => { if (location && map) { map.panTo({ lat: location.latitude, lng: location.longitude }); map.setZoom(15); } else { requestLocation(); } }}>Vị trí của tôi</Button>
				</div>
			</div>
			<MapContainer center={defaultCenter} zoom={12}>
				...
				{showVolunteers && volunteers.map((volunteer) => <VolunteerMarker key={volunteer.id} volunteer={volunteer} onClick={(value) => setSelectedMarker({ type: "volunteer", value })} />)}
				{showRequests && activeRequests.map((request) => <ReliefRequestMarker key={request.id} reliefRequest={request} onClick={(value) => setSelectedMarker({ type: "relief", value })} />)}
			</MapContainer>
			{(isVolunteersLoading || isRequestsLoading) && <div className="map-status"><Spin /> Đang tải dữ liệu bản đồ...</div>}
			{(isVolunteersError || isRequestsError) && <div className="map-status map-error">Không thể tải đầy đủ dữ liệu bản đồ.</div>}
			{locationError && <div className="map-status map-location-error">Không thể lấy vị trí. Hãy cấp quyền vị trí cho trình duyệt rồi bấm “Vị trí của tôi”.</div>}
		</section>
